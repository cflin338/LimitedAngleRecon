...
% figure();
% tiledlayout(1,3);
% nexttile(1); imshow(reshape(arttv_recons(:,end)./max(arttv_recons(:,end)), N, N), 'InitialMagnification','fit');
% nexttile(2); imshow(reshape(artatv_recons(:,end)./max(artatv_recons(:,end)), N, N), 'InitialMagnification', 'fit');
% nexttile(3); imshow(reshape(l1dl2_recons(:,end)./max(l1dl2_recons(:,end)),N,N), 'InitialMagnification','fit');