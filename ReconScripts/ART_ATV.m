function recons = ART_ATV(ProblemSetup, pm_ARTATV)
    ...
    for ang_idx = 1:num_angles
        rows_selected = (ang_idx-1)*bins+1:(ang_idx-1)*bins+bins;
            ...
            D = 1./W_sub; D(isinf(D)) = 0; W_sub = sparse(diag(D));
            M = 1./V_sub; M(isinf(M)) = 0; V_sub = sparse(diag(M));

            A_subsets{ang_idx} = A_sub;
            b_subsets{ang_idx} = b_sub;
            W_subsets{ang_idx} = W_sub;
            V_subsets{ang_idx} = V_sub;
    end
    ...
    visualize         = pm_ARTATV.visualize;
    disp_prog         = pm_ARTATV.disp_prog;
    sample_rate       = pm_ARTATV.sample_rate;
    
    % method specific variables
    iterations        = pm_ARTATV.iterations;
    recon             = pm_ARTATV.initial;
    N_tv              = pm_ARTATV.N_tv;
    alphas            = pm_ARTATV.alphas;
    
    epsilon           = pm_ARTATV.epsilon;
    lambda            = pm_ARTATV.lambda;
    alpha             = pm_ARTATV.alpha;  %TV directions
    thetas            = pm_ARTATV.thetas; %angles
    ...
    D = sum(A,1); D = 1./D; D(isinf(D)) = 0; D = diag(D);
    M = sum(A,2); M = 1./M; M(isinf(M)) = 0; M = diag(M);
    
    W = full(sum(A,2)); 
    V = full(sum(A,1)); 
    for iter = 1:iterations
        if disp_prog
            waitbar((iter-1)/iterations, wb, sprintf('ART-aTV Progress: %i of %i Iterations', ...
                                                 iter, iterations));
        end
        prev_recon = recon;
        ...
        for angle_idx = 1:num_angles
            ...
            A_sub = A_subsets{subset};
            b_sub = b_subsets{subset};
            W_sub = W_subsets{subset};
            V_sub = V_subsets{subset};
